...
import { Wheat, TrendingUp, Calendar, MapPin, Thermometer, Droplets } from 'lucide-react';
...
const CropPrediction = () => {
  ...
  const environmentalFactors = [
    { label: 'Temperature', value: '24°C', icon: Thermometer, color: 'text-orange-600' },
    { label: 'Humidity', value: '65%', icon: Droplets, color: 'text-blue-600' },
    { label: 'Soil pH', value: '6.8', icon: MapPin, color: 'text-green-600' },
    { label: 'Rainfall', value: '45mm', icon: Droplets, color: 'text-cyan-600' },
  ];
    ...
    >
      ...
      <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-lg border border-gray-100">
        <div className="flex items-center space-x-3 mb-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Crop Prediction</h1>
      ...
      {/* Crop Selection */}
      <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-lg border border-gray-100">
        <h2 className="text-xl font-bold text-gray-800 mb-4">Select Crop Type</h2>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
          {crops.map((crop) => (
            ...
            >
              ...
              <p className="font-medium text-gray-800 text-sm sm:text-base">{crop.name}</p>
            </motion.button>
          ))}
        </div>
      </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-green-50 rounded-xl">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">{predictionData.yield}</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-sm text-gray-600">Confidence</p>
                <p className="text-xl font-bold text-green-600">{predictionData.confidence}</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-3 p-4 bg-blue-50 rounded-xl">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-blue-600">{predictionData.harvestDate}</p>
              </div>
            </div>
            
            <div className="p-4 bg-yellow-50 rounded-xl">
              <p className="text-sm text-gray-600 mb-2">Recommendation</p>
              <p className="text-gray-800 font-medium">{predictionData.recommendation}</p>
            </div>
          </div>
        </div>
            ...
            {environmentalFactors.map((factor, index) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="font-semibold text-gray-800">{factor.value}</span>
                </motion.div>
              );
            })}
          </div>
